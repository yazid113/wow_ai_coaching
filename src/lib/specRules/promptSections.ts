...
export function buildGuideSection(guideContext: string): string {
  return [
    ...
    'The following facts were derived directly from the SimulationCraft APL for this spec and patch.',
    'They reflect the actual priority list used by the top theorycraft community.',
    'They OVERRIDE your training data on any point where they conflict.',
    '',
    guideContext,
    '',
    'BEFORE flagging any error you MUST check:',
    "1. Is this ability's priority or condition listed above? Use that.",
    '2. Is this mechanic described above? Use that.',
    '3. If a spell or condition is not mentioned above, be conservative — omit the error rather than guessing.',
    '',
    'When in doubt — DO NOT flag it as an error.',
    'It is better to miss a real error than to flag correct current-patch play as wrong.',
    '',
    '',
  ].join('\n')
}
